"""
Tail-boost ablation for hybrid MC+TTA uncertainty inference.

This wrapper keeps the original inference pipeline intact and only removes the
top-quantile tail boost term from the hybrid variance fusion stage.
"""
...
def _build_hybrid_maps_no_tail_boost(
...
def main() -> None:
    base._build_hybrid_maps = _build_hybrid_maps_no_tail_boost
    args = base.parse_args()
    if args.output_folder is None:
        args.output_folder = base.join(
            base.nnUNet_results,
            f'{args.dataset}/Hybrid_MC_TTA_Risk_NoTailBoost',
        )
    base.run(args)
