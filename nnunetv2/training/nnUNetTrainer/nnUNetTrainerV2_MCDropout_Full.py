...
class nnUNetTrainerV2_MCDropout_Full(nnUNetTrainer):
    """
    nnU-Net Trainer with MC Dropout in both encoder and decoder.

    This trainer injects Dropout3d layers into all StackedConvBlocks and
    supports automatic resume from checkpoint_latest.pth after interruptions.
    """
    ...
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
        ...
        self.dropout_p = 0.5
        # Save latest checkpoint every epoch for robust resume.
        self.save_every = 1
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.print_to_log_file("[MC Dropout] Injecting MC Dropout layers (FULL: ENCODER + DECODER)...")
            dropout_count = self._inject_dropout_to_network()
            self.print_to_log_file(
                f"[SUCCESS] Injected {dropout_count} Dropout3d layers in FULL network (p={self.dropout_p})"
            )
    ...
    def _inject_dropout_to_network(self):
        """
        Inject Dropout3d into all encoder and decoder StackedConvBlocks.
    ...
    def _print_network_structure_sample(self):
        """
        Print a small sample of encoder/decoder blocks to verify full injection.
        """
        network = self._get_actual_network()
